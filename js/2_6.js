...
export function viz_2_6(data, map, options) {
  ...
  d3.csv(
    ...
    if (!d3.select(options.selector + "_summary").empty()) {
      let summary = calendarsummary(summaryData, summary_map, {
        selector: options.selector + "_summary",
      });
    }
  });
  ...
  function getUTCDayDetails(date) {
    ...
    let weekNumber = Math.floor(
      (numberOfDays + startOfYear.getUTCDay() + 1) / 7
    );
  ...
  const square = svg
    ...
    .attr("fill", (d) =>
      map.value != null ? colorScale(d[map.value]) : options.fill
    )
    .attr("stroke", "white")
    ...
    .on("mouseover", (event, d) => {
      tooltip
        ...
        .html(
          `<p>${dateFormat(
            d[map.date]
          )}</p><p>Daily Track Release Count: ${d3.format(",")(
            d[map.value]
          )}</p>`
        )
  ...
  const left_month_line = svg
    .selectAll(".end-month-line")
  ...
  d3.csv(
    ...
    let photoCard = svg
      ...
      .attr("x", (d) => width / 3 + 30)
      .attr("y", (d, i) =>
        i == 0 ? yScale(d.weekNumber) - options.imageSize : yScale(d.weekNumber)
      )
      ...
      .attr(
        "transform-origin",
        (d) =>
          `${width / 2 + 10 + options.imageSize / 2} ${
            yScale(d.weekNumber) + options.imageSize / 2
          }`
      )
      ...
      .on("mouseover", function (event, d) {
        square
          .filter(
            (x) => JSON.stringify(x[map.date]) == JSON.stringify(d[map.date])
          )
          .attr("stroke-width", 3)
          .each(function (d, i) {
        ...
        square.attr("opacity", (x) =>
          JSON.stringify(x[map.date]) == JSON.stringify(d[map.date]) ? 1 : 0.8
        );
      ...
      .on("mouseout", function (event, d) {
        square.attr("stroke-width", 0.8).attr("opacity", 1);
    ...
    let track_details = svg
      ...
      .attr("y", (d, i) =>
        i == 0
          ? yScale(d.weekNumber) - options.imageSize + options.imageSize / 3
          : yScale(d.weekNumber) + options.imageSize / 3
      )
      .attr("font-size", "10")
      .text((d) => d["NAME"])
