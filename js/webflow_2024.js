  ...
  let visuals = {
    ...
    viz_2_15: {
      ...
      options: {
        ...
        text: "white",
        background:
          "https://datacult.github.io/chartmetric-2023/assets/2_15_gradient.svg",
        blend: "soft-light",
      },
      mapping: {
        group: "COUNTRY",
  ...
  document.querySelectorAll(".toggle-wrapper").forEach((toggleWrapper) => {
    toggleWrapper.querySelectorAll(".toggle-switch").forEach((toggleSwitch) => {
      toggleSwitch.addEventListener("click", () => {
        const value = d3.autoType({
          value: toggleSwitch.getAttribute("data"),
        }).value;
        ...
        if (visuals.hasOwnProperty(viz_id)) {
          if (Array.isArray(visuals[viz_id].params[index])) {
            if (visuals[viz_id].params[index].indexOf(value) > -1) {
              ...
              console.log(
                "params detected as an array but value is not an accepted param",
                viz_id,
                index,
                value
              );
            ...
            if (visuals[viz_id].params.indexOf(value) > -1) {
              ...
              console.log(
                "value is not an accepted param",
                viz_id,
                index,
                value
              );
