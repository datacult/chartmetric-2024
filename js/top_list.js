  ...
  function update(newData = data, newMap = map, newOptions = options) {
    ...
    if (map.sort != null) {
      ...
      if (options.sort.length > 0) {
        data = data.sort(
          (a, b) =>
            options.sort.indexOf(a[map.sort]) -
            options.sort.indexOf(b[map.sort])
        );
      }
    }
    ...
    data.forEach((d) => {
      if (!Array.isArray(d[map.genre])) d[map.genre] = [d[map.genre]];
    });
      ...
      .join(
        (enter) => {
          return enter
            ...
            .attr(
              "transform",
              (d) => `translate(${xScale(d[map.x])},${yScale(d[map.y])})`
            );
        },
        (update) => {
          return update
        ...
        (exit) => {
          return exit
            ...
            .attr(
              "transform",
              (d) => `translate(${-width},${yScale(d[map.y])})`
            )
            .remove();
        }
      )
      ...
      .on("mouseover", function (event, d) {
        ...
        tooltip_inner_group
          ...
          .attr("transform", (x) =>
            d == x
              ? `translate(${
                  textWidth + yScale.bandwidth() * (1 + options.padding)
                },0)`
              : `translate(${width + options.margin.left},0)`
          );
      })
      .on("mouseout", function (event, d) {
        ...
        if (
          ...
          tooltip_inner_group
            ...
            .attr(
              "transform",
              (x) => `translate(${width + options.margin.left},0)`
            );
        }
      });

    const tooltip_group = svg
    ...
    const tooltip_inner_group = tooltip_group
      .selectAll(".tooltip_inner_group")
      .data((d) => [d])
      .join("g")
      .attr("transform", (d) => `translate(${width + options.margin.left},0)`)
      .classed("tooltip_inner_group", true);

    const tooltip_rect = tooltip_inner_group
      .selectAll(".tooltip_rect")
      .data((d) => [d])
      .join("rect")
      .attr("x", 0)
      ...
      .attr(
        "width",
        (d) =>
          yScale.bandwidth() * options.padding * 2 +
          options.stage_width +
          options.genre_width +
          options.genre_width_padding * d[map.genre].length
      )
    ...
    const stage_group = tooltip_inner_group
      .selectAll(".stage_group")
      .data((d) => [d])
      .join("g")
      .attr(
        "transform",
        `translate(${yScale.bandwidth() * options.padding},${
          yScale.bandwidth() * (options.padding * 1.5)
        })`
      )
      .classed("stage_group", true);

    const stage_rect = stage_group
    ...
    const stage_text = stage_group
      .selectAll(".stage_text")
      .data((d) => [d])
      .join("text")
    ...
    const genre_group = tooltip_inner_group
      .selectAll(".genre_group")
      .data((d) => [d])
      .join("g")
      .attr(
        "transform",
        `translate(${
          options.stage_width + yScale.bandwidth() * options.padding * 2
        },${yScale.bandwidth() * (options.padding * 1.5)})`
      )
    ...
    const genre_inner_group = genre_group
      ...
      .attr(
        "transform",
        (d, i) => `translate(${i * options.genre_width_padding},${0})`
      )
    ...
    const genre_rect = genre_inner_group
      .selectAll(".genre_rect")
      .data((d) => [d])
      .join("rect")
      .attr("x", 0)
      .attr("y", 0)
    ...
    const genre_text = genre_inner_group
      .selectAll(".genre_text")
      .data((d) => [d])
      .join("text")
      .attr(
        "x",
        (d, i) => options.genre_width / 2 + i * options.genre_width_padding
      )
      .attr("y", yScale.bandwidth() * options.padding)
      .attr("dominant-baseline", "middle")
      .attr("text-anchor", "middle")
    ...
    if (map.image != null) {
      const images = bar_group
        ...
        .data((d) => [d])
        .join("circle")
        .attr("cx", yScale.bandwidth() * 0.6)
        .attr("cy", yScale.bandwidth() * 0.6)
